      ...
      <b-dropdown aria-role="list" position="is-top-right">
        <div class="ws item" :class="loading" slot="trigger" role="button">
          <span v-if="loading === 'connected'">
            <b-icon icon="check-network" size="is-small" />Spawned
          </span>
          <span v-else-if="loading === 'resumed'">
...
type State = 'connected' | 'connecting' | 'disconnected' | 'resumed'
...
export default class Workspace extends Vue {
  ...
  mounted() {
    {
      ...
      //const socket = io('/session', { query: { "device": this.$route.params.device, "bundle": this.$route.params.bundle }, transports: ['websocket'] })
    ...
    this.term.writeln(colors.green('Welcome to Wormhole!'))
  ...
  changed() {
    this.loading = 'connecting'
    this.$ws
      .on('spawned', (data: SpawnedData) => {
          if (data.spawned){
            this.loading = 'connected';
            this.log("white", "Target is running");
            if (data.isApp){
              this.log("warn", "App started but not resumed. Shutting down in few seconds...");
            }
          }
          else{
            this.loading = 'disconnected';
            this.log("error", "An error occurred while spawning or attaching to the target. Try to reload the session")
          }          
      })
      .on('resumed', () => {
        this.log("info", "Target app resumed correctly");
        this.loading = 'resumed';
      })
  ...
  disconnected(extra: string) {
    ...
    this.loading = 'disconnected'*/
    this.log("white", "Target app closed")
    localStorage.clear();
    this.$router.push({
      name: 'Apps',
